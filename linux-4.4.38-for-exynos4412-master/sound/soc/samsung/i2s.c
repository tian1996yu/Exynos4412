...
static inline const struct samsung_i2s_dai_data *samsung_i2s_get_driver_data(
						struct platform_device *pdev)
{
	printk("samsung_i2s_dai_data ty0\n");
	if (IS_ENABLED(CONFIG_OF) && pdev->dev.of_node) {
		printk("samsung_i2s_dai_data ty1\n");
		const struct of_device_id *match;
		match = of_match_node(exynos_i2s_match, pdev->dev.of_node);
		return match ? match->data : NULL;
	} else {
		printk("samsung_i2s_dai_data ty22222222\n");
		return (struct samsung_i2s_dai_data *)
				platform_get_device_id(pdev)->driver_data;
		printk("samsung_i2s_dai_data ty3\n");
	}
}
...
static int samsung_i2s_probe(struct platform_device *pdev)
{
	...
	int ret;
	return 0;
	printk("i2s_probe----------TYTTYTYTY-------------->\n");
	/* Call during Seconday interface registration */
	i2s_dai_data = samsung_i2s_get_driver_data(pdev);

	printk("i2s_probe after------------------------>\n");
...
static struct platform_device tiny4412_iis_dev = {
    .name         = "samsung-i2s",
    .id       = -1,
    .dev = { 
...
struct platform_driver tiny4412_iis_drv = {
	.probe		= samsung_i2s_probe,
	.remove		= samsung_i2s_remove,
	.id_table = samsung_i2s_driver_ids,
	.driver		= {
...
static int tiny4412_iis_init(void)
{
	printk("tiny4412_iis_init-------->\n");
    platform_device_register(&tiny4412_iis_dev);
	printk("tiny4412_iis_init1111111-------->\n");
    platform_driver_register(&tiny4412_iis_drv);
    return 0;
}

static void tiny4412_iis_exit(void)
{
    platform_device_unregister(&tiny4412_iis_dev);
    platform_driver_unregister(&tiny4412_iis_drv);
}

module_init(tiny4412_iis_init);
module_exit(tiny4412_iis_exit);

...
static int soc_bind_dai_link(struct snd_soc_card *card, int num)
{
	...
	printk(card->dev, "ASoC: binding %s at idx %d\n", dai_link->name, num);
...
int snd_soc_register_card(struct snd_soc_card *card)
{
	...
	printk("ty before card->dai_link[0].cpu_dai_name->%s\n",card->dai_link[0].cpu_dai_name);
	for (i = 0; i < card->num_links; i++) {
	...
	printk("ty card->dai_link[0].codec_dai_name->%s\n",card->dai_link[0].codec_dai_name);
	printk("ty card->dai_link[1].codec_dai_name->%s\n",card->dai_link[1].codec_dai_name);
	printk("ty card->dai_link[0].cpu_dai_name->%s\n",card->dai_link[0].cpu_dai_name);
	printk("ty card->dai_link[1].cpu_dai_name->%s\n",card->dai_link[1].cpu_dai_name);
